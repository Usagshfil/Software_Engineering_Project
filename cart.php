...
<div class="container-fluid m-3">	
    ...
    <?php 
    $qry = $conn->query("SELECT c.*,b.image_path,b.name,b.brand FROM cart c inner join products b on b.id = c.product_id where c.customer_id ={$_SESSION['login_id']}");
    $total = 0;
    ?>
    <div class="row">
    <div class="col-md-8">
    			...
    			<li class="list-group-item" data-id="<?php echo $row['id'] ?>" data-price="<?php echo $row['price'] ?>">
    				<div class="d-flex w-100">
    					...
    					<div class="detail-field">
    						<p>Product: <b><?php echo $row['name'] ?></b></p>
...
<script>
    ...
    var totalAmount = $('#tamount').text().replace(/,/g,'');
    $.ajax({
        url: 'vnpay_create_payment.php',
        method: 'POST',
        data: {
            amount: totalAmount,
            language: 'en',
        ...
        success: function(resp){
            ...
            if(response.success){
                alert("Payment created successfully. Transaction ID: " + response.transactionId);
                // Redirect to the sandbox link
				$.ajax({
                ...
                window.location.href = response.sandboxLink;
            } else {

...
Class Action {
	...
	function add_to_cart(){
		...
		$data .= ", qty= $qty";
		$data .= ", price= $price";
		$check = $this->db->query("SELECT * FROM cart where product_id = $product_id and customer_id ={$_SESSION['login_id']} ");
	...
	function get_cart_count(){
		$qry = $this->db->query("SELECT c.*,b.image_path,b.name FROM cart c inner join products b on b.id = c.product_id where c.customer_id ={$_SESSION['login_id']}");
		$data = array();
		$count = 0 ; 
		$data['list']=array();

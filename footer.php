 ...
 <script>
  ...
  window.end_load = function(){
    $('#preloader2').fadeOut('fast', function() {
 	...
 	window.uni_modal = function($name = '' , $url='',$size=''){
    start_load()
    $.ajax({
        url:$url,
        error:err=>{
        ...
        success:function(resp){
            if(resp){
                $('#uni_modal .modal-title').html($name)
  ...
  window.uni_modal_right = function($name = '' , $url=''){
    start_load()
    $.ajax({
        url:$url,
        error:err=>{
        ...
        success:function(resp){
            if(resp){
                $('#uni_modal_right .modal-title').html($name)
          ...
          if(Object.keys(resp.list).length > 0 ){
            ...
            Object.keys(resp.list).map(k=>{
              var li = $('<li class="list-group-item"><div class="item d-flex justify-content-between align-items-center"></div></li>')
               ...
               li.find('.item').append('<div class="cart-title"><b>'+resp.list[k].name+'</b></div>')
